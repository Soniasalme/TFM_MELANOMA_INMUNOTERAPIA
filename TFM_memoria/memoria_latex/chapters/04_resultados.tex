...
El análisis principal se realizó en GSE78220 (25 muestras, tumor, RNA-seq, anti-PD-1). Tras verificar que los datos están en escala FPKM lineal, se aplicó la transformación $\log_2(\text{FPKM}+1)$ y se realizó un filtrado explícito de baja expresión: se retuvieron los genes con FPKM $\geq 1$ en al menos 5 muestras (20\,\% del total), lo que redujo el transcriptoma de 25.268 genes originales a \textbf{14.549 genes testados} (Tabla~\ref{tab:qc_gse78220}). Ambos criterios de filtrado (varianza cero y baja expresión) se aplicaron conjuntamente; los genes con varianza cero constituyen un subconjunto de los eliminados por baja expresión, de modo que el total efectivamente eliminado es 10.719. No se detectaron genes significativos tras corrección FDR, resultado esperable por el tamaño muestral. Se identificaron \textbf{539 genes nominales} ($p < 0{,}05$, $|logFC| \geq 0{,}5$) como candidatos exploratorios.
...
Retirados por baja expresión (no cumplen FPKM $\geq 1$ en al menos 5 muestras) & 10.719 \\
...
\subsection{Validación tumoral NanoString (GSE215868, GSE211645)}
...
\begin{table}[H]
\centering
\caption{Resumen de la validación tumoral NanoString. La firma se calculó con los top 50 genes de GSE78220 presentes en cada panel.}
...
\subsection{Validación tumoral RNA-seq independiente (GSE91061)}

GSE91061 \cite{GSE91061,Riaz2017} (Riaz et al., 2017) es una cohorte tumoral RNA-seq de melanoma metastásico tratado con nivolumab (anti-PD-1). Contiene biopsias pre- y on-treatment de 109 muestras en total; el análisis se restringió a las 51 biopsias pretratamiento para comparabilidad con GSE78220. De estas, 49 muestras disponían de respuesta evaluable (39 no respondedores y 10 respondedores). La firma de descubrimiento alcanzó AUC $= 0{,}474$ con 19 genes disponibles en el solapamiento entre cohortes. La ausencia de señal predictiva clara es coherente con las diferencias entre cohortes: distinto agente anti-PD-1 (nivolumab vs.\ pembrolizumab), heterogeneidad de líneas de tratamiento previas, y posibles diferencias en la definición de respuesta clínica. Este resultado se interpreta como validación exploratoria negativa, no como refutación de la firma.

\begin{table}[H]
\centering
\caption{Validación tumoral RNA-seq en GSE91061. NR: no respondedores; R: respondedores.}
\label{tab:validacion_rnaseq}
\begin{tabularx}{\textwidth}{l r r r r X}
\toprule
Cohorte & $n$ pre-tx & NR/R evaluables & Genes firma & AUC & Consideraciones \\
\midrule
GSE91061 & 51 (49 evaluables) & 39/10 & 19 & 0{,}474 & RNA-seq tumoral; nivolumab; pre-tx \\
\bottomrule
\end{tabularx}
\end{table}

\section{Sensibilidad en sangre periférica}

La firma tumoral anti-PD-1 no mostró señal predictiva en sangre periférica. GSE94873 \cite{GSE94873,Friedlander2017} (360 muestras, NanoString, anti-CTLA-4, sangre total pretratamiento) alcanzó AUC $= 0{,}475$ con \textbf{solo 4 genes de firma con solapamiento} en el panel de 162 sondas; este resultado debe interpretarse con extrema prudencia: con únicamente 4 genes, el score de firma no puede considerarse una evaluación representativa de la firma original y el análisis tiene valor exclusivamente como sensibilidad exploratoria, no como validación formal. La ausencia de señal es coherente con la distancia biológica (firma tumoral vs.\ sangre periférica) y técnica (plataforma RNA-seq vs.\ NanoString, anti-PD-1 vs.\ anti-CTLA-4) entre las cohortes.

\begin{table}[H]
\centering
\caption{Análisis de sensibilidad en sangre periférica. NR: no respondedores; R: respondedores.}
\label{tab:sensibilidad_sangre}
\begin{tabularx}{\textwidth}{l r r r r X}
\toprule
Cohorte & $n$ & NR/R & Genes firma & AUC & Distancia \\
\midrule
GSE94873 & 360 & 314/46 & 4 & 0{,}475 & Firma tumoral→sangre; anti-CTLA-4; NanoString \\
\bottomrule
\end{tabularx}
\end{table}
...
La descarga mediante el paquete TCGAbiolinks \cite{Colaprico2016} proporcionó datos clínicos de 470 pacientes con 90 variables. Las variables de supervivencia se construyeron del siguiente modo: \texttt{OS\_event} = 1 si \texttt{vital\_status = Dead}, 0 si \texttt{vital\_status = Alive}; \texttt{OS\_time} se obtuvo de \texttt{days\_to\_death} en pacientes fallecidos y de \texttt{days\_to\_last\_follow\_up} en los censurados. Tras excluir casos con tiempo negativo o variables ausentes, el conjunto de datos válido para el análisis de supervivencia quedó integrado por 470 pacientes.
...
\end{minipage}
\caption{Izquierda: curva de Kaplan-Meier por sexo (test log-rank, p\,=\,NS). Derecha: curva de Kaplan-Meier por estadio AJCC (I, II, III y IV).}
\label{fig:km_sexo_estadio}
\end{figure}

\subsection{Factores pronósticos clínicos: modelo Cox univariante}

Se ajustaron modelos de Cox univariantes para edad, sexo y estadio AJCC (simplificado en cuatro grupos: I, II, III y IV) utilizando el paquete \texttt{survival} \cite{Therneau2023}. Los resultados se resumen en la Tabla~\ref{tab:cox_univariante_tcga}.
...
Para interpretar biológicamente los genes asociados a la supervivencia, se realizó un análisis de sobrerrepresentación (ORA) sobre GO-BP mediante \texttt{clusterProfiler}, previa conversión de los identificadores Ensembl a Entrez ID con \texttt{org.Hs.eg.db}. Se analizaron por separado los genes protectores (FDR $< 0{,}05$, HR $< 1$) y los genes de riesgo (FDR $< 0{,}05$, HR $> 1$). El análisis de sobrerrepresentación se implementa en el script \texttt{13\_tcga\_anotacion\_ora.Rmd}, siguiendo la misma cadena metodológica aplicada en el análisis de RNA-seq de cáncer colorrectal (objetos SummarizedExperiment, normalización log-CPM, conversión de identificadores Ensembl a Entrez ID con \texttt{org.Hs.eg.db}, y ORA GO-BP con \texttt{clusterProfiler}). Los resultados se presentan a continuación.
